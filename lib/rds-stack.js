...
const { SubnetType, InstanceType, InstanceClass, InstanceSize } = require('@aws-cdk/aws-ec2');
...
class RdsStack extends Stack {
  ...
  createPostgresDb(name, props = {}) {
    ...
    const proxy = new rds.DatabaseProxy(this, Utils.prefixByAppName(name + '-proxy'), {
    ...
    return {
      cluster,
      proxy,
      url: `postgres://${this.dbUser}:${cluster.secret.secretValueFromJson('password')}@${
        proxy.endpoint
