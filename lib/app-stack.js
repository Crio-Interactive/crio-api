...
class AppStack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    const { domain, graphqlEndpointWhole } = this.createWholeLambda({
      env: {
        AWS_COGNITO_USER_POOL_ID: props.userPoolId,
    ...
    const { graphqlLambda: graphqlLambdaStripe } = this.createLambda('stripe', {
      env: {
        CRIO_DATABASE_URL: props.crioDb.url,
        BUCKET_NAME: props.s3Bucket.bucketName,
        BUCKET_URL: 'https://' + props.s3Bucket.bucketDomainName,
    ...
    stripe.addMethod('ANY', stripeIntegration);
  ...
  createWholeLambda({ env, invokeFunctions = [] }) {
    ...
    const prod = graphqlEndpointWhole.root.addResource('prod');
    const graphql = prod.addResource('graphql');
    graphql.addProxy();
    ...
    return { domain, graphqlEndpointWhole };
  }
