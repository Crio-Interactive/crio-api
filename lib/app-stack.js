...
class AppStack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    const { graphqlLambda: graphqlLambdaPrograms } = this.createLambda(
      'programs', {
        env: {
          PROGRAMS_DATABASE_URL: props.programsDb.url,
          BUCKET_NAME: props.s3Bucket.bucketName,
          BUCKET_URL: 'https://' + props.s3Bucket.bucketDomainName,
        },
        invokeFunctions: [
          props.programsDb.proxy.grantConnect.bind(props.programsDb.proxy),
          props.s3Bucket.grantReadWrite.bind(props.s3Bucket),
        ],
      });

    const { graphqlLambda: graphqlLambdaManageUsers } = this.createLambda(
      'manage-users', {
        env: {
          BUCKET_NAME: props.s3Bucket.bucketName,
    ...
    const domain = this.createWholeLambda({
      env: {
        AWS_COGNITO_USER_POOL_ID: props.userPoolId,
        PROGRAMS_URL: graphqlLambdaPrograms.functionName,
        UPLOAD_URL: graphqlLambdaUpload.functionName,
        MANAGE_USERS_URL: graphqlLambdaManageUsers.functionName,
      },
      invokeFunctions: [
        this.grantCognitoFullAccess.bind(this),
        wholeLambda => {
          this.grantInvoke(wholeLambda, [
            graphqlLambdaPrograms,
            graphqlLambdaUpload,
  ...
  createWholeLambda({ env, invokeFunctions = [] }) {
    ...
    const graphqlEndpointWhole = new apiGateway.LambdaRestApi(
      this,
      'graphqlEndpointWhole',
      {
        domainName: {
          domainName: domain,
          certificate,
    ...
    new route53.ARecord(this, 'ApiAliasRecord', {
      ...
      target: route53.RecordTarget.fromAlias(
        new targets.ApiGateway(graphqlEndpointWhole),
      ),
    });
  ...
  createLambda(
    serviceName,
    { env = {}, customProps = {}, invokeFunctions = [] },
  ) {
